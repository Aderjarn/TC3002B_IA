...
model_predict_dir = os.path.join('Predict')
model_predict_datagen = ImageDataGenerator(
    rescale=1./255,
    preprocessing_function=tf.image.rgb_to_grayscale
    )
model_predict_generator = model_predict_datagen.flow_from_directory(
    external_dir,
    target_size=(64, 64),
    batch_size=1,
    class_mode='sparse',
...
model_predict = new_model.predict(model_predict_generator)

# Map predicted class indices to class names
predicted_class_names = [class_names[np.argmax(prediction)] for prediction in model_predict]

import matplotlib.pyplot as plt

# Function to display images with their predicted classes
def display_external_images_with_predictions(generator, predictions, class_names):
    plt.figure(figsize=(15, 15))
    num_images = len(predictions)
...
# Display external images with their predicted classes
display_external_images_with_predictions(model_predict_generator, model_predict, class_names)